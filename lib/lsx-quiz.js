...
class Exercise {
    ...
    render() {
        // Render overall structure
        if (!this.renderedScore1 || !this.renderedMain) {
            let points = calcScorePointsHtml(this.plugin);
            let hx = this.plugin.config.quizExerciseHeading;

            [this.renderedScore1, this.renderedMain] = parseHtml(`
        ...
        if (!this.renderedScore2 || !this.renderedHint) {
            [this.renderedScore2, this.renderedHint] = parseHtml(`
                <div class="lsx-quiz-column-score"></div>
                <div class="lsx-quiz-column-main lsx-quiz-hint"></div>
            `);
        }

        updateScorePoints({
            element:     this.renderedScore1,
            maxScore:    this.maxScore,
            actualScore: this.actualScore,
        });
        ...
        if (exerciseNumber === 1) {
            this.renderedScore1.classList.add("lsx-quiz-border-top");
            this.renderedScore1.classList.remove("pt-4");
            this.renderedMain.classList.remove("pt-4");
        } else {
            this.renderedScore1.classList.remove("lsx-quiz-border-top");
            this.renderedScore1.classList.add("pt-4");
        ...
        // Render questions
        let result = [this.renderedScore1, this.renderedMain];

        if (this.hint) {
            result.push(this.renderedScore2, this.renderedHint);
        }
...
class Question {
    ...
    render() {
        ...
        this.renderedMain.innerHTML = `<div class="lsx-quiz-question">${this.text}</div>`;;
        this.renderedHint.innerHTML = this.hint;
...
class GapTextQuestion extends Question {
    ...
    static Line = class extends QuestionLine {
        ...
        renderMain() {
            ...
            let renderChildren = (childNodes) => {
                ...
                for (let childNode of childNodes) {
                    ...
                    if (gap) {
                        ...
                        if (childNode.replaceChildren) {
                            let renderedChildren = renderChildren([...childNode.childNodes]);
                            childNode.replaceChildren(...renderedChildren);
                        }
                    }
                }
            ...
            let renderedChildren = renderChildren(this.childNodes);
            this.rendered.replaceChildren(...renderedChildren);
