...
class Exercise {
    ...
    render() {
        ...
        if (!this.renderedScore || !this.renderedMain) {
            ...
            this.renderedTitle = this.renderedMain.querySelector(":scope > *");
        }
...
class Question {
    ...
    static createFromHtmlSource(questionElement, plugin, exercise) {
        ...
        for (let questionTextElement of questionElement.querySelectorAll(":scope > lsx-question-text")) {
            question.text += " " + questionTextElement.innerHTML;
            questionTextElement.remove();
        }

        for (let questionHintElement of questionElement.querySelectorAll(":scope > lsx-question-hint")) {
    ...
    evaluateAnswers() {
        ...
        for (let line of this.lines) {
            ...
            this.actualScore += Math.max(0, line.actualScore);
        }
    }
...
class QuestionLine {
    ...
    readStatusAndPoints(element, defaultStatus) {
        let result = {
            ...
            points: parseInt(element.getAttribute("points")) || NaN,
        };
    ...
    static Line = class extends QuestionLine {
        ...
        static createFromHtmlSource(questionLineElement, plugin, question) {
            ...
            for (let answerElement of questionLineElement.querySelectorAll(":scope > lsx-answer")) {
                let {status, points} = questionLine.readStatusAndPoints(answerElement, this.STATUS_UNKNOWN);
            ...
            if (!answer.rendered.container || !answer.rendered.button || !answer.rendered.label) {
                answer.rendered.container = parseHtml(`
                    <div class="lsx-quiz-choice-answer" data-enabled="">
                        ...
                        <div class="lsx-quiz-choice-label"></div>
                    </div>
                `)[0];
...
class AssignmentQuestion extends Question {
    ...
    parseHtmlSource(questionElement, plugin) {
        ...
        this.emptyPoints    = parseInt(questionElement.getAttribute("empty-points")) || 0;
        this.wrongPoints    = parseInt(questionElement.getAttribute("wrong-points")) || 0;
    ...
    static Line = class extends QuestionLine {
        ...
        static createFromHtmlSource(questionLineElement, allAssignmentElements, plugin, question) {
            ...
            for (let assignmentElement of questionLineElement.querySelectorAll(":scope > lsx-assignment")) {
                ...
                evaluationRule[questionLine.question.emptyAnswer] = {
                ...
                for (let otherAssignmentElement of allAssignmentElements) {
                    let answer = (otherAssignmentElement.getAttribute("answer") || "").trim();
                    if (!answer) continue;

                    evaluationRule[answer] = {
                        status: questionLine.STATUS_WRONG,
                        points: questionLine.question.wrongPoints,
                    }

                    if (otherAssignmentElement === assignmentElement || !otherAssignmentElement.innerHTML.trim()) {
                        let status = otherAssignmentElement === assignmentElement ? questionLine.STATUS_CORRECT : questionLine.STATUS_WRONG;
                        evaluationRule[answer] = questionLine.readStatusAndPoints(assignmentElement, status);
                    }
                }
        ...
        evaluateAnswers() {
            ...
            for (let assignment of this.assignments) {
                // Evaluate answer
                let evaluation = assignment.evaluationRule[assignment.selectedAnswer];
                assignment.currentEvaluation.status = evaluation.status;
                assignment.currentEvaluation.points = evaluation.points;
                ...
                for (let key of Object.keys(assignment.evaluationRule)) {
                    maxScore = Math.max(maxScore, assignment.evaluationRule[key].points);
                }
            ...
            while (linebreak && oldParentElement) {
                ...
                for (let childNode of [...oldParentElement.childNodes]) {
                    if (!moveNodes && childNode.nodeType === Node.TEXT_NODE && childNode.textContent.includes(linebreak)) {
                        ...
                        for (let textLine of textLines.slice(1)) {
        ...
        if (this.mode === "split-lines") {
            ...
            for (let questionLineElement of questionElement.querySelectorAll(":scope > lsx-question-line")) {
                if (questionLineElement.firstChild.nodeType === Node.TEXT_NODE) {
                    ...
                    let oldLength = questionLineElement.firstChild.textContent.length;
                    questionLineElement.firstChild.textContent = questionLineElement.firstChild.textContent.trimStart();
                    let newLength = questionLineElement.firstChild.textContent.length;

                    if (newLength < oldLength) {
                        // Unicode code point 160: No-Break Space
                        questionLineElement.firstChild.textContent = String.fromCodePoint(160).repeat(oldLength - newLength) + questionLineElement.firstChild.textContent;
                    }
                }
            }
        }
