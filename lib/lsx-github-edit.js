...
* © 2020 – 2026 Dennis Schulmeister-Zimolong <dennis@pingu-mail.de>
...
export default class LSX_GithubEdit {
    ...
    preprocessHtml(html, plugin) {
        ...
        let downloadUrlPrefix = plugin.config.githubDownloadUrlPrefix;
        ...
        let onOpen            = plugin.config.githubEditOnOpen;
        ...
        lsxElements.forEach(lsxElement => {
            ...
            let editLink     = `${editUrlPrefix}${url}`;
            let downloadLink = `https://dennisschulmeister.github.io/DownGit/#/home?url=${downloadUrlPrefix}${download}`;
            let pagesLink1   = `${pagesUrlPrefix}${pagesUrl1}`;
            let pagesLink2   = `${pagesUrlPrefix}${pagesUrl2}`;
            let pagesLink3   = `${pagesUrlPrefix}${pagesUrl3}`;

            let editButton = `
                <a href="${editLink}" target="_blank" class="_edit">
            ...
            let pagesButton1 = `
                <a href="${pagesLink1}" target="_blank" class="_pages1">
            ...
            let pagesButton2 = `
                <a href="${pagesLink2}" target="_blank" clas="_pages2">
                    <button class="btn btn-light">
                        ${pagesLabel2}
                    </button>
                </a>`;
            ...
            let pagesButton3 = `
                <a href="${pagesLink3}" target="_blank" class="_pages3">
                    <button class="btn btn-light">
                        ${pagesLabel3}
                    </button>
                </a>`;
            ...
            newElement.querySelector("._edit").addEventListener("click", () => {
                if (onOpen) {
                    onOpen(url, editLink);
                }
            });
